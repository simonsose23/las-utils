...
class ScalarClassClusterer():
    ...
    '''
    def execute(self, las, cluster_dict, scalar_field, rename=None, default_class=None):
        pr = las.points
...
def main(cfg: DictConfig) -> None:
    ...
    clusterer = ScalarClassClusterer()
    ...
    for tgt, src in progressbar(zip(tgt_las_files, src_las_files)):
        with laspy.open(src) as fh:
            ...
            if 'cluster' in cfg:
                clusterer.execute(las, cfg['cluster'], cfg['scalar_field'], rename=cfg['rename'], default_class=cfg['default_class'])
